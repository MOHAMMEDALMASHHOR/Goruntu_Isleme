# -*- coding: utf-8 -*-
"""
Created on Sun Jan  5 12:18:50 2025
...
def detect_bright_areas(frame):
    ...
    value_channel = hsv_frame[:, :, 2]  # Extract the brightness (value) channel

    # Define a threshold for brightness detection (adjust as needed)
    brightness_threshold = 230
    bright_mask = cv2.inRange(value_channel, brightness_threshold, 255)

    # Calculate the percentage of bright pixels
    bright_area_ratio = np.sum(bright_mask > 0) / (frame.shape[0] * frame.shape[1]) * 100
    
    # Adjust brightness if too much light is detected
    if bright_area_ratio > 5:  # If more than 5% of the frame is too bright
        ...
        print(f"Detected bright areas. Reducing screen brightness to {new_brightness}%")
...
while cap.isOpened():
    ...
    bright_mask = detect_bright_areas(frame)
